...
def get_data():
    ...
    archived_data = ingestion_obj.get_sql_table(table_name="archived_data")
    archived_data_selected = archived_data.iloc[: int(len(archived_data) * 0.9)]
...
def make_report(results):
    ...
    if target_drifts:
        for target in target_drifts:
            ...
            cur = train_data_transformed[
                [col for col in train_data_transformed.columns if col != target]
            ]
            ...
            report = Report(
                metrics=[DataQualityPreset(), DataDriftPreset(), TargetDriftPreset()]
            )
...
def monitoring_pipeline(smtp_server="smtp.office365.com", smtp_port=587):

...
def test_feature_importance_plot(model_trainer, mock_savefig):  # pylint: disable=W0613
    ...
    with patch('matplotlib.pyplot.savefig'):
        ...
        model_trainer.data.feature_names = ['feature_0', 'feature_1', 'feature_2', 'feature_3',
...
def test_train_model(mock_rf, mock_data, model_trainer):  # pylint: disable=W0613
    ...
    with patch('src.components.model_trainer.RandomForestRegressor',
               return_value=mock_rf) as mock_rf_constructor:

        model_trainer.train_model('rf', mock_best_params)

    mock_rf_constructor.assert_called_once_with(n_estimators=100, max_depth=10)
    mock_rf.fit.assert_called_once_with(model_trainer.data.x_train, model_trainer.data.y_train)
...
def test_initiate_model_training(model_trainer, mock_savefig):  # pylint: disable=W0613
    ...
    with patch('optuna.create_study', return_value=mock_study), \
         patch.object(model_trainer, 'train_model') as mock_train_model, \
         patch('sklearn.metrics.mean_absolute_error',
               side_effect=lambda x, y, **kwargs: np.abs(x - y).mean()):

        mock_predictions = np.random.rand(len(model_trainer.data.x_test),
                                          len(model_trainer.data.y_test[0]))
        mock_model = Mock()
        mock_model.predict.return_value = mock_predictions
        mock_train_model.return_value = mock_model
        mock_model.feature_importances_ = np.random.rand(len(model_trainer.data.feature_names))
        ...
        with patch('mlflow.start_run'), \
             ...
             patch('sklearn.preprocessing.StandardScaler.inverse_transform',

...
def fixture_data_transformation_obj():
    ...
    obj.transformation_config.feature_scaler_path = os.path.join(
        get_project_root(), "tests", "test_data", "test_feature_scaler.pkl"
    )
    obj.transformation_config.target_scaler_path = os.path.join(
...
def test_data_transformation(_, data_transformation_obj):
    ...
    random_offline_data = {
        ...
        "timestamp_probeentnahme": [
            random.randint(1e5, 1e9) for _ in range(num_offline_samples)
        ],
        "timestamp_messung": [
            random.randint(1e5, 1e9) for _ in range(num_offline_samples)
        ],
        ...
        "bemerkungen": [
            random.choice(["S1", "W1", "Test"]) for _ in range(num_offline_samples)
        ],
        "aktivsauerstoff": [
            random.uniform(0.02, 0.12) for _ in range(num_offline_samples)
        ],
        "anionischetenside": [
            random.uniform(0.26, 0.35) for _ in range(num_offline_samples)
        ],
        ...
        "leitfaehigkeit": [
            random.uniform(380, 550) for _ in range(num_offline_samples)
        ],
        "nichtionischentenside": [
            random.uniform(0.02, 0.08) for _ in range(num_offline_samples)
        ],
        "oberflaechenspannung": [
            random.randint(30, 39) for _ in range(num_offline_samples)
        ],
        "peressigsaeure": [
            random.uniform(0.10, 0.15) for _ in range(num_offline_samples)
        ],
        ...
        "wasserstoffperoxid": [
            random.randint(35, 43) for _ in range(num_offline_samples)
        ],
    ...
    random_online_data = {
        ...
        "timestamp": [random.randint(1e5, 1e9) for _ in range(num_online_samples)],
    ...
    assert (
        len(x_train[(x_train < 0) & (x_train > 1)]) == 0
    ), "x_train are not in 0 and 1 range"
    assert (
        len(y_train[(y_train < 0) & (y_train > 1)]) == 0
    ), "y_train are not in 0 and 1 range"
    ...
    for row in x_test:
        assert not np.any(
            np.all(row == x_train, axis=1)
        ), "x_train data points found in x_test"
    for row in y_test:
        assert not np.any(
            np.all(row == y_train, axis=1)
        ), "y_train data points found in y_test"
